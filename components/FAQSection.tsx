      ...
      <div className="container mx-auto px-4 max-w-4xl">
        {showTitle && (
          <div className="text-center mb-12">
            <div className="inline-flex items-center gap-2 bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-200 dark:border-indigo-500/20 rounded-full px-3.5 py-1.5 mb-4 text-xs font-semibold text-indigo-700 dark:text-indigo-300">
            ...
            <p className="mt-3 text-slate-600 dark:text-slate-400 text-sm md:text-base max-w-xl mx-auto">
        ...
        <div className="space-y-4">
          {FAQ_ITEMS.map((item, index) => {
            ...
            return (
              <div
                key={index}
                className="bg-white dark:bg-slate-900/70 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden transition-all duration-200 hover:border-indigo-400 dark:hover:border-slate-700 shadow-sm dark:shadow-none"
              >
                <button
                  onClick={() => toggleItem(index)}
                  className="w-full text-left px-6 py-5 flex items-center justify-between gap-4 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 group"
                  aria-expanded={isOpen}
                  id={`faq-toggle-${index}`}
                >
                  <span className="font-semibold text-slate-900 dark:text-white text-base md:text-lg group-hover:text-indigo-600 dark:group-hover:text-indigo-300 transition-colors">
                    {item.question}
                  </span>
                  <div className={`p-1.5 rounded-lg bg-slate-100 dark:bg-slate-800/80 text-slate-500 dark:text-slate-400 shrink-0 transition-all duration-200 ${isOpen ? 'rotate-180 text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950/40' : 'group-hover:bg-slate-200 dark:group-hover:bg-slate-800'}`}>
                    <ChevronDown className="w-4 h-4" />
                  </div>
                </button>

                {isOpen && (
                  <div className="px-6 pb-6 pt-1 text-slate-600 dark:text-slate-300 text-sm leading-relaxed border-t border-slate-100 dark:border-slate-800/50">
                    {item.answer}
                  </div>
                )}
              </div>
        ...
        <div className="mt-10 p-5 rounded-2xl bg-gradient-to-r from-slate-50 via-indigo-50/40 to-slate-50 dark:from-slate-900 dark:via-indigo-950/30 dark:to-slate-900 border border-indigo-100 dark:border-indigo-500/20 text-center flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="text-left">
            <h4 className="text-sm font-bold text-slate-900 dark:text-white">Still have questions?</h4>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">We're here to help with your account, billing, and business plans.</p>
          </div>
          <a
            href={createSupportMailto('general')}
